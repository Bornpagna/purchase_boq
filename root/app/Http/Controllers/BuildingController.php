...
class BuildingController extends Controller
{
    ...
    public function index(){
		$data = [
			'title'			=> trans('lang.building'),
			'icon'			=> 'fa fa-square',
			'small_title'	=> trans('lang.building_list'),
			'background'	=> '',
			'link'			=> [
				'home'	=> [
						'url' 		=> url('/'),
		...
		if(hasRole('building_add')){
			$data = array_merge($data, ['rounteSave'=> url('building/save')]);
		}
		if(hasRole('building_download')){
			$data = array_merge($data, ['rounteDownload'=> url('building/excel/download')]);
		}
		if(hasRole('building_upload')){
			$data = array_merge($data, ['rounteUploade'	=> url('building/excel/upload')]);
		}
	...
	public function getDt(Request $request)
    {
		...
		->addColumn('action', function ($row) {			
			$rounte_boq = url('building/upload/'.$row->id);
			$rounte_delete = url('building/delete/'.$row->id);
			$rounte_edit = url('building/edit/'.$row->id);
			$btnEdit = 'onclick="onEdit(this)"';
			$btnDelete = 'onclick="onDelete(this)"';
			
			if(!hasRole('building_edit')){
				$btnEdit = "disabled";
			}
			if(House::where(['building_id'=>$row->id])->exists()){
				$btnDelete = "disabled";
			}
			if(!hasRole('building_delete')){
				$btnDelete = "disabled";
			}
            return
    ...
    public function downloadExcel(Request $request)
   	{
		$pro_id = $request->session()->get('project');
   		Excel::create('building.export_'.date('Y_m_d_H_i_s'),function($excel)use($pro_id){
