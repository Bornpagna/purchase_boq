...
class ReturnUsageController extends Controller
{
    ...
    public function GetDetail(Request $request)
    {
    	try {
			...
			$OrderItem  = ReturnUsageDetails::select($select)
						->leftJoin('items','return_usage_details.item_id','items.id')
						->where('return_usage_details.return_id',$request->id)
						->where('return_usage_details.delete',0)
						->get();
    ...
    public function index(){
		$data = [
			...
			'rounte'		=> url("stock/reuse/dt"),
		];
		
		if(hasRole('usage_return_add')){
			$data = array_merge($data, ['rounteAdd'=> url('stock/reuse/add')]);
		}
		return view('stock.usage.return.index',$data);
	}
	...
	public function add(Request $request){
		$data = [
			...
			'rounteSave'	=> url('stock/reuse/save'),
			'rounteBack'	=> url('stock/reuse'),
			'pro_id'		=> $request->session()->get('project'),
		];
		return view('stock.usage.return.add',$data);
	}
	...
	public function edit(Request $request, $id){
		...
		if($obj){
			$data = [
				...
				'rounteSave'	=> url('stock/reuse/update/'.$id),		
				'rounteBack'	=> url('stock/reuse'),		
				'obj'	=> $obj,
				'pro_id'=> $request->session()->get('project'),
			];
			return view('stock.usage.return.edit',$data);
		}else{
	...
	public function getDt(Request $request)
    {
		...
		->addColumn('details_url',function($row){
            return url('stock/reuse/subdt').'/'.$row->id;
        })
		->addColumn('action', function ($row) {
			$rounte_delete = url('stock/reuse/delete/'.$row->id);
			$rounte_edit = url('stock/reuse/edit/'.encrypt($row->id));
    ...
    public function save(Request $request){
		...
		try {
			...
			if(count($request['line_index']) > 0){
				for($i=0;$i<count($request['line_index']);$i++){
					...
					if (getSetting()->is_costing==1) {
						...
						$streetID = $request['line_street'][$i];
						$houseID = $request['line_on_house'][$i];
						$warehouseID = $request['line_from_warehouse'][$i];
			...
			if($request->btnSubmit==1){
				return redirect('stock/reuse')->with('success',trans('lang.save_success'));
			}
