...
class BlockController extends Controller
{
    ...
    public function index(){
		$data = [
			'title'			=> trans('lang.block'),
			'icon'			=> 'fa fa-square',
			'small_title'	=> trans('lang.block_list'),
			'background'	=> '',
			'link'			=> [
				'home'	=> [
						'url' 		=> url('/'),
		...
		if(hasRole('block_add')){
			$data = array_merge($data, ['rounteSave'=> url('block/save')]);
		}
		if(hasRole('block_download')){
			$data = array_merge($data, ['rounteDownload'=> url('block/excel/download')]);
		}
		if(hasRole('block_upload')){
			$data = array_merge($data, ['rounteUploade'	=> url('block/excel/upload')]);
		}
	...
	public function getDt(Request $request)
    {
		...
		->addColumn('action', function ($row) {			
			$rounte_boq = url('block/upload/'.$row->id);
			$rounte_delete = url('block/delete/'.$row->id);
			$rounte_edit = url('block/edit/'.$row->id);
