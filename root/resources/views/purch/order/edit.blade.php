					...
					<div class="row">
						...
						<div class="col-md-4">
							<div class="form-group">
								...
								<input class="form-control reference_no" length="20" type="text" id="reference_no" name="reference_no" value="{{$obj->ref_no}}" placeholder="{{ trans('lang.enter_text') }}">
...
<script type="text/javascript">	
	...
	function initTable(data){
		...
		$.ajax({
			url :'{{url("purch/request/remotePREdit")}}',
			type:'POST',
			data:{
				'_token': _token,
				'pr_id': data.pr_id,
			...
			success:function(pr_qty){
				if(pr_qty){
					$(".line_qty_orig_"+data.id).val(pr_qty);
				}
			},error:function(){
	...
	$(document).ready(function(){
		...
		$('.pr_no').empty();
		$('.pr_no').append(new Option('','',false,false));
		$('.pr_no').append(new Option('{{$obj->ref_no}}','{{$obj->pr_id}}',true,true));
		$("#pr_no").select2('val', '{{$obj->pr_id}}');
