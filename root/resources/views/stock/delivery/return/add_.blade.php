						...
						<table class="table table-striped table-bordered table-hover" width="100%" id="table-income">
							<thead>
								<tr style="font-size:12px;">
									...
									<th width="4%" class="text-center all">{{trans('lang.action')}}</th>
								</tr>
							</thead>
							<tbody>
								<tr>
...
<script type="text/javascript">	
	...
	function initTable(data){
		$("#table-income tbody").append('<tr>'+
				...
				'	<input type="hidden" value="0" class="line_qty_stock line_qty_stock_'+data.id+'"/>'+
				'</td>'+
				'<td>'+
				...
				'</td>'+
				'<td>'+
				'	<input type="number" step="any" value="'+data.disc_usd+'" length="11" class="form-control noscroll line_refund line_refund_'+data.id+'" onkeyup="calculateRecord('+data.id+')" name="line_refund[]"/>'+
				'</td>'+
				'<td>'+
				'	<input type="text" readonly value="'+data.total+'" class="form-control line_total line_grand_total_'+data.id+'" name="line_total[]"/>'+
				'</td>'+
				'<td>'+
				'	<select class="form-control line_warehouse line_warehouse_'+data.id+'" name="line_warehouse[]" onchange="remoteStockItem('+data.id+')">'+
				'		<option value=""></option>'+
				'		{{getWarehouse()}}'+
				'	</select>'+
				'</td>'+
	...
	$(document).ready(function(){
		...
		setInterval(function() {
			$.ajax({
				url :'{{url("prefix/gen/RED/ReturnDelivery/ref_no")}}',
				type:'get',
				success:function(data){
					if(data!=null && data!=''){
		...
		$("#deliv_no").on('change', function(){
			var val = $(this).val();
			if(val!=null && val!=''){
				var _token = $("input[name=_token]").val();
				$.ajax({
					url :'{{url("stock/deliv/remoteDelivery")}}',
					type:'POST',
					data:{
						'_token': _token,
						'del_id': val,
					...
					success:function(data){
						if(data){
							var sub_total = data.head.sub_total;
							var refund = data.head.disc_usd;
							var grand_total =data.head.grand_total;
							...
							var str = '';
								str+= '<tr>';
								str+='<th colspan="9" class="text-right" style="vertical-align: middle !important;">{{trans("lang.sub_total")}}</th>';
								...
								str+='</tr>';
								str+='<tr>';
								str+='<th colspan="9" class="text-right" style="vertical-align: middle !important;">{{trans("lang.refund")}}</th>';
								str+='<th colspan="2" class="text-right"><div class="input-group input-icon right"><span class="input-group-addon"><i class="fa fa-dollar font-red"></i></span><input type="number" value="'+refund+'" class="noscroll refund form-control " onkeyup="totalRefund(this)" id="refund" name="refund"/></div></th>';
								str+='</tr>';
								str+='<tr>';
								str+='<th colspan="9" class="text-right" style="vertical-align: middle !important;">{{trans("lang.grand_total")}}</th>';
