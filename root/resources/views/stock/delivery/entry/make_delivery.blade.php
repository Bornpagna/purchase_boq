...
<script type="text/javascript">	
	...
	function onSetItem(val, row){
		...
		if(val!=null && val!='' && jsonUnits[row] && jsonItems){
			$.each(jsonUnits[row].filter(d=>d.to_code==val.unit_stock), function(k, v){
	...
	function initTable(data,deliver_address){
		...
		var str = "";
			...
			str += '</td>';
			str += '<td>';
			str += '<input type="text" value="'+isNullToString(data.desc)+'" length="100" class="form-control line_reference line_reference_'+data.id+'" name="line_reference[]"/>';
			str += '</td>';
			str += '</tr>';
