				...
				<table class="table table-striped table-bordered table-hover" id="my-table">
					<thead>
						<tr>
							...
							<th width="20%" class="text-center all">{{ trans('lang.reference') }}</th>
        ...
        str += '<table class="table table-striped details-table table-responsive"  id="sub-'+d.id+'">';
            str += '<thead>';
                str += '<tr>';
                    ...
                    str += '<th style="width: 15%;">{{trans("lang.units")}}</th>';
                    str += '<th style="width: 15%;">{{trans("lang.qty_adjust")}}</th>';
                    str += '<th style="width: 15%;">{{trans("lang.warehouse")}}</th>';
                    str += '<th style="width: 30%;">{{trans("lang.note")}}</th>';
                str += '</tr>';
            str += '</thead>';
        str +='</table>';
        return str;
	...
	function initTable(tableId, data) {
		$('#' + tableId).DataTable({
			...
			columns: [
				...
				{ data: 'item', name: 'item' },
				{ data: 'unit', name: 'unit' },
				{ data: 'adjust_qty', name: 'adjust_qty' },
				{ data: 'warehouse', name: 'warehouse' },
				{ data: 'note', name: 'note' }
			],fnCreatedRow:function(nRow, aData, iDataIndex){
				$('td:eq(3)',nRow).html(formatNumber(aData['adjust_qty']));
			}
		});
	}
