...
<script type="text/javascript">	
	...
	$('#save_close').on('click',function(){
		...
		if(chkValid([".reference_no",".trans_date",".reference",".trans_desc",".line_from_warehouse",".line_on_house",".line_street",".line_item",".line_unit",".line_return_qty",<?php if(getSetting()->return_constructor==1){echo '".sub_const",';} ?>".engineer"])){
			if(isSave()){
	...
	function initTable(data){
		$("#table-income tbody").append('<tr>'+
				...
				'</td>'+
				'<td>'+
				'	<select class="form-control line_from_warehouse line_from_warehouse_'+data.id+'" name="line_from_warehouse[]">'+
				'		<option value=""></option>'+
				'		{{getWarehouse()}}'+
				'	</select>'+
				'</td>'+
		...
		$(".line_on_house_"+data.id+",.line_from_warehouse_"+data.id+",.line_street_"+data.id+",.line_unit_"+data.id).select2({placeholder:'{{trans("lang.please_choose")}}',width:'100%',allowClear:'true'});
