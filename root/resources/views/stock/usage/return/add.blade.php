								...
								<div class="portlet-body" style="background: #f8f9fb;padding: 12px;border: 1px solid #72aee2;">
									...
									<div class="row">
										<div class="col-md-12">
											...
											@endif
											<textarea class="form-control trans_desc" id="trans_desc" name="desc" rows="<?php if(getSetting()->return_constructor==1){echo '3';}else{echo '5';} ?>" length="100" placeholder="{{ trans('lang.enter_text') }}"> </textarea>
										</div>
									</div>
								</div>
							</div>
							...
							<table class="table table-striped table-responsive table-bordered table-hover" width="100%" id="table-income">
								<thead>
									<tr style="font-size:12px;">
										...
										<th width="10%" class="text-center all">{{ trans('lang.from_house') }}</th>
...
<script type="text/javascript">	
	...
	$('#save_close,#save_new').on('click',function(){
		...
		if(chkValid([".reference_no",".trans_date",".reference",".trans_desc",".line_from_warehouse",".line_street",".line_on_house",".line_item",".line_unit",".line_return_qty",<?php if(getSetting()->return_constructor==1){echo '".sub_const",';} ?>".engineer"])){
			if(isSave()){
	...
	function onChangeUnit(field, row){
		...
		var unit = $(field).val();
		if(on_house_id!=null && on_house_id!='' && item_id!=null && item_id!='' && unit!=null && unit!=''){
			...
			$.ajax({
				...
				success:function(data){
					$('.line_usage_qty_'+row).val(data.usage_qty);
		...
		$(".check_row_"+row).val(on_house_id+"_"+item_id);
	}
	...
	$("#btnAdd").on('click',function(){
		...
		if(line_row<=99){
			...
			$("#table-income tbody").append('<tr>'+
					...
					'</td>'+
					'<td>'+
					'	<input type="number" length="50" step="any" class="form-control line_return_qty line_return_qty_'+i+'" name="line_return_qty[]" onkeyup="enterQtyUsage(this, '+i+')" />'+
					'	<input type="hidden" class="form-control line_usage_qty line_usage_qty_'+i+'" name="line_usage_qty[]"/>'+
					'	<input type="hidden" class="form-control line_boq_set line_boq_set_'+i+'" name="line_boq_set[]"/>'+
					'</td>'+
					'<td>'+
					'	<select class="form-control line_from_warehouse line_from_warehouse_'+i+'" name="line_from_warehouse[]">'+
					'		<option value=""></option>'+
					'		{{getWarehouse()}}'+
					'	</select>'+
					'</td>'+
			...
			$(".line_return_qty_"+i).ForceNumericOnly();
	...
	$(document).ready(function(){
		...
		setInterval(function() {
			$.ajax({
				url :'{{url("prefix/gen/REU/ReturnUsage/ref_no")}}',
				type:'get',
				success:function(data){
					if(data!=null && data!=''){
