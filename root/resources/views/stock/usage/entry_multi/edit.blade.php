								...
								<div class="portlet-body" style="background: #f8f9fb;padding: 12px;border: 1px solid #72aee2;">
									...
									<div class="row">
										<div class="col-md-12">
											...
											@endif
											<div class="form-group">
												<label for="warehouse_id" class="col-md-3 control-label" style="text-align: left;"><strong>{{ trans('lang.warehouse') }}</strong></label>
												<div class="col-md-9">
													...
													@else
														<select class="form-control warehouse_id" id="warehouse_id" name="warehouse_id" onchange="onChangeWarehouse()">
															<option value=""></option>
															{{getWarehouse()}}
														</select>
													@endif
...
<script type="text/javascript">	
	...
	function GetDetail(id) {
		return $.ajax({url:'{{url("/stock/use/GetDetail")}}',type:'GET',dataType:'json',data:{id:id},async:false}).responseJSON;
	}
	...
	$('#save_close').on('click',function(){
		...
		if(chkValid([".reference_no",".trans_date",".reference",".trans_desc",".line_on_house",".line_street",".line_item",".line_unit",".line_use_qty",<?php if(getSetting()->usage_constructor==1){echo '".sub_const",';} ?>".engineer"])){
			if(isSave()){
	...
	function onChangeStreet(field){
		...
		if(val!=null && val!='' && jsonHouse){
			...
			$.each(jsonHouse.filter(c=>c.street_id == val), function(key ,value){
	...
	function onChangeUnit(field, row, old_qty){
		...
		var unit = $(field).val();
		if(warehouse_id!=null && warehouse_id!='' && house_id!=null && house_id!='' && item_id!=null && item_id!='' && unit!=null && unit!='' && trans_date!=null && trans_date!=''){
			...
			$.ajax({
				...
				success:function(data){
					$('.line_stock_qty_'+row).val(parseFloat(data.stock_qty) + parseFloat(old_qty));
					if(parseFloat(data.boq_set)!=-1){
						$('.line_boq_set_'+row).val(parseFloat(data.boq_set) + parseFloat(old_qty));
					}else{
						$('.line_boq_set_'+row).val(parseFloat(data.boq_set));
					}
				},error:function(){
		...
		$(".check_row_"+row).val(warehouse_id+"_"+house_id+"_"+item_id);
	}
	...
	function initTable(data){
		$("#table-income tbody").append('<tr>'+
				...
				'	<input type="hidden" class="check_row check_row_'+data.id+'" value="'+data.warehouse_id+'_'+data.house_id+'_'+data.item_id+'" />'+
				...
				'</td>'+
				'<td>'+
				'	<input type="number" length="50" step="any" class="form-control line_use_qty line_use_qty_'+data.id+'" name="line_use_qty[]" onkeyup="enterQtyUsage(this, '+data.id+')" />'+
		...
		$(".line_street_"+data.id+",.line_unit_"+data.id).select2({placeholder:'{{trans("lang.please_choose")}}',width:'100%',allowClear:'true'});
		$(".line_use_qty_"+data.id).ForceNumericOnly();
		...
		$(".line_use_qty_"+data.id).val(data.qty);
	...
	$(document).ready(function(){
		$.fn.select2.defaults.set("theme", "classic");
		$(".engineer, .sub_const, .warehouse_id, .house_id, .street_id").select2({placeholder:'{{trans("lang.please_choose")}}',width:'100%',allowClear:'true'});
		...
		$("#engineer").select2('val','{{$obj->eng_usage}}');
		$("#sub_const").select2('val','{{$obj->sub_usage}}');
