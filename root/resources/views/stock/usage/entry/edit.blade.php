								...
								<div class="portlet-body" style="background: #f8f9fb;padding: 12px 12px 0 12px	;border: 1px solid #72aee2;">
									...
									<div class="row" >
										<div class="col-md-12">
											...
											<div class="form-group">
												<label for="warehouse_id" class="col-md-4 control-label"><strong>{{ trans('lang.warehouse') }}</strong></label>
												<div class="col-md-8">
...
<script type="text/javascript">	
	...
	function onChangeUnit(field, row, old_qty){
		...
		if(from_warehouse_id!=null 
			...
			&& on_house_id!='' 
			&& item_id!=null 
			&& item_id!='' && unit!=null && unit!=''&& trans_date!=null && trans_date!=''){
	...
	function initTable(data){
		$("#table-income tbody").append('<tr>'+
				...
				'</td>'+
				'<td>'+
				'	<select class="form-control line_street line_street_'+data.id+'" onchange="onChangeStreet(this, '+data.id+')" name="line_street[]">'+
				'		<option value=""></option>'+
				'		{{getSystemData("ST")}}'+
				'	</select>'+
				'</td>'+
				'<td>'+
				'	<select class="form-control line_on_house line_on_house_'+data.id+'" onchange="onChangeHouse(this, '+data.id+')" name="line_on_house[]">'+
				'		<option value=""></option>'+
				'	</select>'+
				'</td>'+
				'<td>'+
		...
		$(".line_on_house_"+data.id+",.line_from_warehouse_"+data.id+",.line_street_"+data.id+",.line_unit_"+data.id+",.line_item_"+data.id).select2({placeholder:'{{trans("lang.please_choose")}}',width:'100%',allowClear:'true'});
		...
		$(".line_from_warehouse_"+data.id).select2('val', data.warehouse_id);
		$(".line_street_"+data.id).select2('val', data.street_id);
		$(".line_on_house_"+data.id).select2('val', data.house_id);
