...
<script type="text/javascript">	
	...
	$('#save_close,#save_new').on('click',function(){
		...
		if(chkValid([".reference_no",".trans_date",".reference",".trans_desc","warehouse_id",".street_id",".house_id",".line_item",".line_unit",".line_return_qty",<?php if(getSetting()->return_constructor==1){echo '".sub_const",';} ?>".engineer"])){
			if(isSave()){
	...
	$("#btnAdd").on('click',function(){
		...
		if (street!=null && street!='' && house!=null && house!='' && warehouse!=null && warehouse!='') {
			$('#row-default').remove();
