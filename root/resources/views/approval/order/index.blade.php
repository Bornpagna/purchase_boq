				...
				<table class="table table-striped table-bordered table-hover" id="my-table">
					<thead>
						<tr>
							...
							<th width="10%" class="text-center all">{{ trans('lang.pr_no') }}</th>
							...
							<th width="12%" class="text-center all">{{ trans('lang.action') }}</th>
						</tr>
					</thead>
					<tbody></tbody>
				</table>
...
<script type="text/javascript">	
	var objRole = JSON.parse(convertQuot("{{\App\Model\Role::where(['zone'=>2])->get(['id','name','min_amount','max_amount','level','dep_id'])}}"));
	var objApproval = JSON.parse(convertQuot("{{\App\Model\ApproveOrder::select('po_id','role_id','approved_date','approved_by',DB::raw('(SELECT pr_users.`name` FROM pr_users WHERE pr_users.`id`=approved_by)AS approve_name'))->where('reject',0)->get()}}"));
	...
	var my_table = $('#my-table').DataTable({
		...
		columns: [
			...
			$('td:eq(4)',nRow).html(formatDate(aData['delivery_date'])).addClass("text-right");
