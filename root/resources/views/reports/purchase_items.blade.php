		...
		<div class="portlet light bordered">
			...
			<div class="portlet-body">
				...
				<table class="table table-striped table-bordered table-hover dt-responsive" id="my-table1">
					<thead>
						<tr>
                            <th width="8%" class="all">{{ trans('lang.trans_ref') }}</th>
							<th width="8%" class="all">{{ trans('lang.trans_date') }}</th>
                            <th width="22%" class="all">{{ trans('lang.items') }}</th>
                            <th width="10%" class="all">{{ trans('lang.item_type') }}</th>
                            <th width="8%" class="all">{{ trans('lang.pr_qty') }}</th>
                            <th width="8%" class="all">{{ trans('lang.close_pr') }}</th>
                            <th width="8%" class="all">{{ trans('lang.po_qty') }}</th>
                            <th width="8%" class="all">{{ trans('lang.close_po') }}</th>
                            <th width="8%" class="all">{{ trans('lang.delivery_qty') }}</th>
...
<script type="text/javascript">
    $("#btnSearch").on('click',function(){
        if (chkValid([".item_id"])) {
            ...
            $.ajax({
                url :'{{url("report/purchase_items/get_data")}}',
                type:'POST',
                data:{
                    '_token': _token,
                ...
                success:function(data){
                    ...
                    $.each(data, function(k, v){
                        str += '<tr>';
                        str += '    <td>'+v.ref_no+'</td>';
                        str += '    <td class="text-center">'+formatDate(v.trans_date)+'</td>';
                        str += '    <td>'+v.item_name+'</td>';
                        str += '    <td>'+v.cat_name+'</td>';
                        str += '    <td>'+formatNumber(v.pr_qty)+'</td>';
                        str += '    <td>'+formatNumber(v.pr_close_qty)+'</td>';
                        str += '    <td>'+formatNumber(v.po_qty)+'</td>';
                        str += '    <td>'+formatNumber(v.po_close_qty)+'</td>';
                        str += '    <td>'+formatNumber(v.deliv_qty)+'</td>';
                        str += '    <td class="text-center">'+v.unit_stock+'</td>';
                        str += '</tr>';                                
                    });
    ...
    function clearTable(){
        $(".table-row, .invoice-table-tbody").empty();
        $(".table-row, .invoice-table-tbody").append('<tr><td colspan="9" class="text-center">{{trans('lang.no_data_available_in_table')}}</td></tr>');
    }

	$(document).ready(function(){
        $('#item_id').select2({width:'100%',placeholder:'{{trans("lang.please_choose")}}',allowClear:true});
