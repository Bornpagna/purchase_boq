                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th style="width: 15%;" class="upper-text center">{{trans('lang.warehouse')}}</th>
                            <th style="width: 10%;" class="upper-text center">{{trans('lang.category')}}</th>
                            <th style="width: 30%;" class="upper-text center">{{trans('lang.item_code')}}</th>
                            <th style="width: 35%;" class="upper-text center">{{trans('lang.item_name')}}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th style="width: 15%;">{{trans('lang.item_type')}}</th>
                            <th style="width: 20%;">{{trans('lang.item_code')}}</th>
                            <th style="width: 30%;">{{trans('lang.item_name')}}</th>
                            <th style="width: 8%;">{{trans('lang.on_hand')}}</th>
                            <th style="width: 8%;">{{trans('lang.unit')}}</th>
                            <th style="width: 8%;">{{trans('lang.avg_cost')}}</th>
                            <th style="width: 8%;">{{trans('lang.asset_value')}}</th>
                        </tr>
                    </thead>
                    <tbody>
                    ...
                    @foreach($report as $i => $row)
                        ...
                        <tr>
                            ...
                            <!-- Type -->
                            @if($category = SystemData::find($row->cat_id))
                            <td>{{$category->name}}</td>
                            @else
                            <td></td>
                            @endif
                            ...
                            <td>{{$row->name}}</td>
                            <!-- On Hand -->
                            <td class="right">{{formatQty($row->exact_qty)}}</td>
                            <!-- Unit -->
                            <td class="right">{{$row->to_desc}}</td>
                            <!-- Avg Cost -->
                            <?php $avgCost = ($row->asset_value > 0 ? ((float)$row->exact_qty / (float)$row->asset_value) : 0);?>
                            <td class="right">{{formatDollars($avgCost)}}</td>
                            <!-- Asset Value -->
                            <td class="right">{{formatDollars($row->asset_value)}}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
