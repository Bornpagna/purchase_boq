                                ...
                                <table class="table table-striped table-bordered table-hover" id="data-table">
                                    <thead>
                                        <tr style="font-size:12px;">
                                            ...
                                            <th width="36%" class="text-center all">{{ trans('lang.item_name') }}</th>
                                            ...
                                            <th width="10%" class="text-center all">{{ trans('lang.asset_value') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <th class="text-center" colspan="7">{{ trans('lang.no_data_available_in_table') }}</th>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
...
<script type="text/javascript">
    ...
    initDataTable = function(){
        ...
        var table = $('#data-table').DataTable({
			...
			ajax: '<?php echo url("report/inventory/generateInventoryValuationSummary").$param;?>&version=datatables',
			columns: columns,
            'search':{'regex':true},
            order:[0,'desc'],
            fnCreatedRow:function(nRow,aData,iDataIndex){
                ...
                $('td:eq(3)',nRow).html(formatNumber(exactQty)).addClass('text-right');
                $('td:eq(5)',nRow).html(formatDollar(avgCost)).addClass('text-right');
                $('td:eq(6)',nRow).html(formatDollar(assetValue)).addClass('text-right');
			}
		});
    }
    ...
    onPrint = function(args){
		...
		if (version=='print') {
            var mywindow = window.open("<?php echo url("report/inventory/printInventoryValuationSummary").$param;?>", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,width=800,height=800");
            setInterval(function(){ 
                mywindow.print();
                mywindow.close();
            }, 5000);
        }else if(version=='excel'){
			window.location.href="<?php echo url("report/inventory/generateInventoryValuationSummary").$param;?>&version="+version;
		}
    }
