				...
				<table class="table table-striped table-bordered table-hover" id="my-table">
					<thead>
						<tr>
                            ...
                            <th width="30%" class="all phone-l phone-p">{{ trans('lang.item_name') }}</th>
                            <th width="5%" class="all phone-l phone-p">{{ trans('lang.qty') }}</th>
                            <th width="5%" class="all phone-l phone-p">{{ trans('rep.ordered_qty') }}</th>
                            <th width="5%" class="all phone-l phone-p">{{ trans('rep.delivery_qty') }}</th>
...
<script type="text/javascript">
	...
	function generatePrint(response) {
        if (response) {
            ...
            $.each(response,function(key,v){
                ...
                divString += '<td style="text-align:center !important;" class="black-all">'+v.request_name+'('+v.request_nick+')</td>';
                ...
                divString += '<td style="text-align:center !important;" class="black-all">'+v.deliv_qty+'</td>';
	...
	function onPrint(argument) {
		...
		if (version=='print') {
			$.ajax({
				url:'<?php echo url("/report/purchase-request-and-order-delivery-data").$param;?>&version='+version,
				type:'GET',
			...
			window.location.href="<?php echo url("/report/purchase-request-and-order-delivery-data").$param;?>&version="+version;
		}
	}
	...
	$(document).ready(function(){
		...
		$('#my-table').DataTable({
			...
			processing: true,
			serverSide: true,
			ajax: '<?php echo url("/report/purchase-request-and-order-delivery-data").$param;?>&version=datatables',
			columns: [
