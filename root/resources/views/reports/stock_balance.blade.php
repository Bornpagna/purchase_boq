				...
				<table class="table table-striped table-bordered table-hover dt-responsive" id="my-table1">
					<thead>
						<tr>
							...
							<th width="10%" class="text-center all">{{ trans('lang.code') }}</th>
...
<script type="text/javascript">
    $("#btnSearch").on('click',function(){
        ...
        $.ajax({
            url :'{{url("report/stock_balance/get_data")}}',
            type:'POST',
            data:{
                '_token': _token,
            ...
            success:function(data){
                ...
                $.each(data, function(key, val){
	                ...
	                str += '    <td>'+val.item_code+'</td>';
	                str += '    <td>'+val.item+'</td>';
	                str += '    <td>'+val.cat_name+'</td>';
	                str += '    <td class="text-center">'+val.unit_stock+'</td>';
	                str += '    <td>'+formatNumber(val.begin_balance)+'</td>';
	                str += '    <td>'+formatNumber(val.stock_in)+'</td>';
	                str += '    <td>'+formatNumber(val.stock_out)+'</td>';
	                str += '    <td>'+formatNumber(val.ending_balance)+'</td>';
    ...
    $("#btnExcel").on('click',function(){
        ...
        window.location.href = "<?php echo url('report/stock_balance/get_data')?>?"+data;
    });
