                                ...
                                <table class="table table-striped table-bordered table-hover dt-responsive" id="data-table">
                                    <thead>
                                        <tr style="font-size:12px;">
                                            ...
                                            <th width="10%" class="text-center all">{{ trans('lang.boq_add') }}</th>
                                            <th width="10%" class="text-center all">{{ trans('lang.request_qty') }}</th>
                                            <th width="5%" class="text-center all">{{ trans('lang.boq_unit') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <th class="text-center" colspan="8">{{ trans('lang.no_data_available_in_table') }}</th>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
...
<script type="text/javascript">
    ...
    initDataTable = function(){
        ...
        var columns = [];
        columns.push({data: 'trans_date', name:'trans_date'});
        ...
        columns.push({data: 'qty_request', name:'qty_request'});
        columns.push({data: 'unit', name:'unit'});
        var table = $('#data-table').DataTable({
			"lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "{{trans("lang.all")}}"]],
			processing: true,
			serverSide: true,
			ajax: '<?php echo url("report/boq/generateRemainingBOQTotal").$param;?>&version=datatables',
			columns: columns,
    ...
    onPrint = function(args){
		...
		if (version=='print') {
            var mywindow = window.open("<?php echo url("report/boq/printRemainingBOQTotal").$param;?>", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,width=800,height=800");
            setInterval(function(){ 
                mywindow.print();
                mywindow.close();
            }, 20000);
        }else if(version=='excel'){
			window.location.href="<?php echo url("report/boq/generateRemainingBOQTotal").$param;?>&version="+version;
		}
    }
