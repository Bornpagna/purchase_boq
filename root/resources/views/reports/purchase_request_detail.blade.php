				...
				<table class="table table-striped table-bordered table-hover" id="my-table">
					<thead>
						<tr>
							...
							<th width="5%" class="all">{{ trans('lang.desc') }}</th>
						</tr>
					</thead>
					<tbody></tbody>
				</table>
    ...
    <div class="invoice-items">
        <div class="div-table">
            ...
            <th width="5%" class="all">{{ trans('lang.trans_date') }}</th>
						...
						<th width="10%" class="none">{{ trans('lang.request_by') }}</th>
						<th width="5%" class="all phone-l phone-p">{{ trans('lang.item_type') }}</th>
						<th width="5%" class="all phone-l phone-p">{{ trans('lang.item_code') }}</th>
						<th width="15%" class="all phone-l phone-p">{{ trans('lang.item_name') }}</th>
						<th width="5%" class="all phone-l phone-p">{{ trans('lang.qty') }}</th>
						<th width="5%" class="all phone-l phone-p">{{ trans('lang.units') }}</th>
						<th width="5%" class="all phone-l phone-p">{{ trans('lang.boq') }}</th>
						<th width="5%" class="all">{{ trans('lang.status') }}</th>
						<th width="15%" class="all">{{ trans('lang.desc') }}</th>
                    </tr>
                </thead>
...
<script type="text/javascript">
	...
	function onPrint(argument) {
		...
		if (version=='print') {
			$.ajax({
				url:'<?php echo url("/report/purchase-detail-data").$param;?>&version='+version,
				type:'GET',
			...
			window.location.href="<?php echo url("/report/purchase-detail-data").$param;?>&version="+version;
		}
	}
	...
	$(document).ready(function(){
		...
		$('#my-table').DataTable({
			...
			processing: true,
			serverSide: true,
			ajax: '<?php echo url("/report/purchase-detail-data").$param;?>&version=datatables',
			columns: [
				...
				$('td:eq(9)',nRow).html(boq_set).addClass("text-center");
				$('td:eq(10)',nRow).html(trans_status).addClass("text-center");
				$('td:eq(11)',nRow).html(aData['desc']);
