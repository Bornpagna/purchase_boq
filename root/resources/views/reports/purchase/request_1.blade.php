...
<?php 
    ...
    if (Request::input('pr_id')) {
        $pr_id = '';
        foreach (Request::input('pr_id') as $jkey => $jv) {
            $pr_id.=','.$_POST['pr_id'][$jkey];
            $prArray[$jkey] = $_POST['pr_id'][$jkey];
        }
        $param.='&pr_id='.$pr_id;
    }else{
        $param.='&pr_id='.$pr_id;
	                    ...
	                    <div class="portlet-body form-horizontal" style="border: 1px solid #72aee2;padding: 5px 0px;background: #f8f9fb;">
	                        <div class="col-md-12">
	                            <div class="form-group">
	                                ...
	                                <div class="col-md-3">
	                                    <label for="pr_id" class="control-label bold">{{trans('lang.reference_no')}}</label>
	                                    <select class="form-control" id="pr_id" name="pr_id[]" multiple>
	                                        <option value=""></option>
	                                    </select>
	                                </div>
	                                <div class="col-md-3">
				...
				<table class="table table-striped table-bordered table-hover dt-responsive" id="my-table">
					<thead>
						<tr>
							...
							<th width="10%" class="none">{{ trans('rep.project') }}</th>
							<th width="10%" class="all">{{ trans('lang.reference_no') }}</th>
							<th width="10%" class="all">{{ trans('rep.order_code') }}</th>
							<th width="10%" class="all phone-l phone-p">{{ trans('lang.department') }}</th>
							<th width="10%" class="all phone-l phone-p">{{ trans('lang.request_by') }}</th>
							<th width="10%" class="all phone-l phone-p">{{ trans('lang.ordered_by') }}</th>
							<th width="10%" class="all phone-l phone-p">{{ trans('lang.supplier') }}</th>
							<th width="10%" class="all phone-l phone-p">{{ trans('lang.grand_total') }}</th>
							<th width="10%" class="all">{{ trans('lang.status') }}</th>
						</tr>
					</thead>
					<tbody></tbody>
				</table>
    ...
    <div class="invoice-items">
        <div class="div-table">
            <table class="invoice-table">
                <thead>
                    <tr>
                        <th width="3%" class="all">{{ trans('lang.no') }}</th>
						<th width="14%" class="all phone-l phone-p">{{ trans('lang.reference_no') }}</th>
						<th width="13%" class="all phone-l phone-p">{{ trans('rep.order_code') }}</th>
						<th width="10%" class="all phone-l phone-p">{{ trans('lang.trans_date') }}</th>
						<th width="10%" class="all phone-l phone-p">{{ trans('lang.department') }}</th>
						<th width="10%" class="all phone-l phone-p">{{ trans('lang.request_by') }}</th>
						<th width="10%" class="all phone-l phone-p">{{ trans('lang.ordered_by') }}</th>
						<th width="10%" class="all phone-l phone-p">{{ trans('lang.grand_total') }}</th>
...
<script type="text/javascript">
	...
	var prObj = JSON.parse(convertQuot('{{\App\Model\Request::select(['id','ref_no','trans_status'])->where('trans_status','!=',4)->get()}}'));

	function initRequest(field) {
		$(field).empty();
		$(field).append($('<option></option>').val('').text(''));
		if (prObj.length > 0) {
			$.each(prObj,function(key,val){
				$(field).append($('<option></option>').val(val.id).text(val.ref_no));
			});
		}
	}
	...
	function generatePrint(response) {
        if (response) {
            ...
            $.each(response,function(key,val){
                ...
                divString += '<td style="text-align:center !important;" class="black-all">'+val.pr_no+'</td>';
                divString += '<td style="text-align:center !important;" class="black-all">'+val.po_no+'</td>';
                ...
                divString += '<td style="text-align:center !important;" class="black-all">'+(val.request_people_name!=null ? val.request_people_name+' - '+val.request_people_code : '')+'</td>';
                divString += '<td style="text-align:center !important;" class="black-all">'+(val.order_people_name!=null ? val.order_people_name : '')+'</td>';
                divString += '<td style="text-align:center !important;" class="black-all">'+formatDollar(val.grand_total)+'</td>';
	...
	function onPrint(argument) {
		...
		if (version=='print') {
			$.ajax({
				url:'<?php echo url("/report/purchase/request/generate_request_1").$param;?>&version='+version,
				type:'GET',
			...
			window.location.href="<?php echo url("/report/purchase/request/generate_request_1").$param;?>&version="+version;
		}
	}
	...
	$(document).ready(function(){
        ...
        $('#dep_id,#trans_status,#pr_id').select2({width:'100%',placeholder:'{{trans("lang.please_choose")}}',allowClear:true});
        ...
        $('#pr_id').select2('val',<?php echo json_encode($prArray);?>);

		$('#my-table').DataTable({
			"lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "{{trans("lang.all")}}"]],
			processing: true,
			serverSide: true,
			ajax: '<?php echo url("/report/purchase/request/generate_request_1").$param;?>&version=datatables',
			columns: [
				...
				{data: 'order_people_name', name:'order_people_name'},
				...
				$('td:eq(9)',nRow).html(trans_status).addClass("text-center");
