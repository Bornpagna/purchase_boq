				...
				<table class="table table-striped table-bordered table-hover dt-responsive" id="my-table">
					<thead>
						<tr>
							...
							<th width="10%" class="all phone-l phone-p">{{ trans('lang.request_by') }}</th>
							<th width="10%" class="all phone-l phone-p">{{ trans('lang.delivery_date') }}</th>
							<th width="10%" class="all phone-l phone-p">{{ trans('lang.note') }}</th>
            ...
            <table class="invoice-table">
                <thead>
                    <tr>
						...
						<th width="14%" class="all phone-l phone-p">{{ trans('lang.trans_date') }}</th>
						<th width="13%" class="all phone-l phone-p">{{ trans('lang.reference_no') }}</th>
						<th width="10%" class="all phone-l phone-p">{{ trans('lang.department') }}</th>
						<th width="10%" class="all phone-l phone-p">{{ trans('lang.request_by') }}</th>
						<th width="10%" class="all phone-l phone-p">{{ trans('lang.delivery_date') }}</th>
						<th width="20%" class="all phone-l phone-p">{{ trans('lang.note') }}</th>
						<th width="10%" class="all">{{ trans('lang.status') }}</th>
                    </tr>
                </thead>
...
<script type="text/javascript">
        ...
        if (response) {
            ...
            $.each(project,function(k,v){
            	if (v) {
	                ...
	                divString += '<td style="text-align:left;border:1px solid #fff !important;font-weight:bold;">'+v.project+'</td>';
	                divString += '<td style="text-align:left;border:1px solid #fff !important;"></td>';
	                divString += '<td style="text-align:right;border:1px solid #fff !important;font-weight:bold;"></td>';
	                divString += '<td style="text-align:left;border:1px solid #fff !important;"></td>';
	                divString += '<td style="text-align:right;border:1px solid #fff !important;font-weight:bold;"></td>';
	                divString += '<td style="text-align:left;border:1px solid #fff !important;"></td>';
	                divString += '<td style="text-align:left;border:1px solid #fff !important;"></td>';
	                divString += '</tr>';
            		$.each(response.filter(a=>a.pro_id==v.pro_id),function(key,val){
		                ...
		                divString += '<td style="text-align:center !important;" class="black-all">'+formatDate(val.delivery_date)+'</td>';
	...
	function onPrint(argument) {
		...
		if (version=='print') {
			$.ajax({
				url:'<?php echo url("/report/purchase/request/generate_request_2").$param;?>&version='+version,
				type:'GET',
			...
			window.location.href="<?php echo url("/report/purchase/request/generate_request_2").$param;?>&version="+version;
		}
	}
	...
	$(document).ready(function(){
		...
		$('#my-table').DataTable({
			...
			processing: true,
			serverSide: true,
			ajax: '<?php echo url("/report/purchase/request/generate_request_2").$param;?>&version=datatables',
			columns: [
				...
				{data: 'request_people_code', name:'request_people_code'},
				...
				$('td:eq(8)',nRow).html(trans_status).addClass("text-center");
				$('td:eq(0)',nRow).html(formatDate(aData['trans_date'])).addClass("text-center");
				$('td:eq(6)',nRow).html(formatDate(aData['delivery_date'])).addClass("text-center");
			}
		});
	});
</script>
