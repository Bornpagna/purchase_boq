				...
				<table class="table table-striped table-bordered table-hover" id="my-table">
					<thead>
						<tr>
							...
							<th width="12%" class="text-center all">{{ trans('lang.grand_total') }}</th>
...
<script type="text/javascript">	
        ...
        str += '<table class="table table-striped details-table table-responsive"  id="sub-'+d.id+'">';
            str += '<thead>';
                str += '<tr>';
                    ...
                    str += '<th style="width: 10%;">{{trans("lang.to_warehouse")}}</th>';
                    str += '<th style="width: 10%;">{{trans("lang.note")}}</th>';
                str += '</tr>';
            str += '</thead>';
        str +='</table>';
        return str;
	...
	var my_table = $('#my-table').DataTable({
		...
		columns: [
			...
			$('td:eq(5)',nRow).html(formatDollar(aData['sub_total']));
			$('td:eq(6)',nRow).html(formatDollar(aData['disc_usd']));
			$('td:eq(7)',nRow).html(formatDollar(aData['grand_total']));
