				...
				<table class="table table-striped table-bordered table-hover dt-responsive" id="my-table">
					<thead>
						<tr>
							...
							<th width="15%" class="text-center desktop">{{ trans('lang.cheque_name') }}</th>
							<th width="15%" class="text-center">{{ trans('lang.tel') }}</th>
							<th width="25%" class="text-center desktop">{{ trans('lang.address') }}</th>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#btnAdd").on("click",function(){
			...
			$('.supplier-form').attr('action',rounte);
			...
			$('#sup-desc').val(' ');
			$('#address').val(' ');
			$('.button-submit-supplier').attr('id','btnSave').attr('name','btnSave');
			$('.button-submit-supplier').html('{{trans("lang.save")}}');
			$('.supplier-modal').modal('show');
			
			$("#btnSave").on('click',function(){
				if(chkValid([".sup-name",".tel",".sup-desc",".status",".address"])){
					if(chkDublicateName(objName, '#sup-name')){
	...
	function onEdit(field){
		...
		$('.supplier-form').attr('action',rounte);
		$('.supplier-modal').children().find('div').children().find('h4').html('{{trans("lang.edit")}}');
		if(objName){
			$.each(objName.filter(c=>c.id==id),function(key,val){
				$('#old_name').val(val.name);
				$('#sup-name').val(val.name);
				$('#tel').val(val.tel);
				$('#sup-desc').val(val.desc);
				$('#status').select2('val',val.status);
				$('#address').val(val.address);
			});
		}
		$('.button-submit-supplier').attr('id','btnUpdate').attr('name','btnUpdate');
		$('.button-submit-supplier').html('{{trans("lang.save_change")}}');
		$('.supplier-modal').modal('show');
		
		$("#btnUpdate").on('click',function(){
			if(chkValid([".sup-name",".tel",".sup-desc",".status",".address"])){
				if(chkDublicateName(objName, '#sup-name','#old_name')){
					$('.supplier-form').submit();
				}else{
					return false;
				}
