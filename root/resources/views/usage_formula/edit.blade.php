			...
			<div class="portlet-body">
                ...
                <form id="frmUsagePolicy" class="form-horizontal" method="POST" action="{{$route}}" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <table class="table table-striped table-bordered table-hover" id="my-table">
                                ...
                                <tbody>
                                    @if($usageFormulaDetail)
                                        <?php $index = 0; ?>
                                        @foreach($usageFormulaDetail as $row)
                                        <?php $index++; ?>
                                        <tr class="tr-{{$row->id}}">
                                            <td class="text-center house house-{{$row->id}}">{{($index)}}</td>
                                            @if(getSetting()->allow_zone == 1)
                                                <td class="text-center zone zone-{{$row->id}}">
                                                {{($usageFormula->zone ? $usageFormula->zone : '')}}
                                                <input type="hidden" name="line_zone_id[]" value="{{$row->zone_id}}"/>
                                                </td>
                                            @endif
                                            @if(getSetting()->allow_block == 1)
                                                <td class="text-center block block-'+ value.id +'">
                                                {{$usageFormula->block ? $usageFormula->block : ''}}
                                                <input type="hidden" name="line_block_id[]" value="{{$row->block_id}}"/>
                                                </td>
                                            @endif
                                            
                                            <td class="text-center street street-{{$row->id}}">
                                            {{$usageFormula->street ? $usageFormula->street : ''}}
                                            <input type="hidden" name="line_street_id[]" value="{{$row->street_id}}"/>
                                            </td>
                                            <td class="text-center house_type-{{$row->id}}">
                                            {{$row->house_Type}}
                                            <input type="hidden" name="line_house_type[]" value="{{$row->house_type}}"/>
                                            </td>
                                            <td class="text-center house house-{{$row->id}}">
                                            {{$row->house_no}}
                                            <input type="hidden" name="line_house_id[]" value="{{$row->id}}"/>
                                            </td>
                                            <td class="percent percent-{{$row->id}}">
                                            <input type="number" min="1" max="100" required class="form-control input-percent input-percent-{{$row->id}}" id="line_percent.{{$index}}" name="line_percent[]" value="{{$row->percentage}}" />
                                            </td>
                                            <td class="delete delete-{{$row->id}}">
                                            <button title="{{trans('lang.remove')}}" onclick="onRemoveRowClicked('{{$row->id}}');" class="btn btn-sm red"><i class="fa fa-trash"></i> {{trans("lang.remove")}}</button>
                                            </td>
                                            </tr>
                                        @endforeach
                                    @endif
                    ...
                    <div class="form-actions text-right">
						<button onclick="onSubmit();" type="button" id="update" name="update" value="1" class="btn green bold">{{trans('lang.edit')}}</button>
						<a class="btn red bold" rounte="{{$routeBack}}" id="btnCancel">{{trans('lang.cancel')}}</a>
					</div>
                </form>
			</div>
		</div>
...
<script type="text/javascript">
    ...
    getZonesComplete = function(response){
        if('{{isset($usageFormula)}}' == 1){
            $(".zone_id").select2('val', "{{$usageFormula->zone_id}}");
        }
    }
    ...
    getBlocksComplete = function(response){
        if('{{isset($usageFormula)}}' == 1){
            $(".block_id").select2('val', "{{$usageFormula->block_id}}");
        }
    }
    ...
    getStreetsComplete = function(response){
        if('{{isset($usageFormula)}}' == 1){
            $(".street_id").select2('val', "{{$usageFormula->street_id}}");
        }
    }
