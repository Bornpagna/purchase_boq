...
class CreateStockBalanceDetailStoredProcedures extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        GROUP BY C.id) ;
      END";
        \DB::unprepared("DROP PROCEDURE IF EXISTS stock_balance_detail; CREATE PROCEDURE stock_balance_detail(IN `pro_` INT, IN `warehouse_` VARCHAR(100), IN `start_` VARCHAR(100), IN `end_` VARCHAR(100), IN `item_` INT) {$sql}");
    }
    ...
    {
        \DB::unprepared("DROP PROCEDURE IF EXISTS stock_balance_detail;");
    }
}
