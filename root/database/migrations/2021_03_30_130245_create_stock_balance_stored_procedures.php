...
class CreateStockBalanceStoredProcedures extends Migration
{
    /**
     * Run the migrations.
     *
          ...
          FROM
            ...
            FROM
              ...
              WHERE st.`pro_id` = pro_ 
                ...
                AND st.`delete` = 0) AS A) AS B) AS C 
      GROUP BY C.warehouse_id,
          C.item_id)AS D WHERE D.qty > 0) ;
        END IF ;
        END IF ;
      END";
        \DB::unprepared("DROP PROCEDURE IF EXISTS stock_balance; CREATE PROCEDURE stock_balance(IN `pro_` INT, IN `warehouse_` VARCHAR(100), IN `start_` VARCHAR(100), IN `end_` VARCHAR(100), IN `group_` INT, IN `is_zero` INT) {$sql}");
    }
    ...
    {
        \DB::unprepared("DROP PROCEDURE IF EXISTS stock_balance;");
    }
}
