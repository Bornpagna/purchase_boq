...
class CreateAllStockTransactionStoredProcedures extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    FROM
      ...
      INNER JOIN `pr_stocks` AS st 
        ...
        END IF;
        END";
        \DB::unprepared("DROP PROCEDURE IF EXISTS all_stock_transaction; CREATE PROCEDURE all_stock_transaction(IN `pro_` INT, IN `warehouse_` VARCHAR(100), IN `start_` VARCHAR(100), IN `end_` VARCHAR(100), IN `type_` INT) {$sql}");
    }
    ...
    {
        \DB::unprepared("DROP PROCEDURE IF EXISTS all_stock_transaction;");
    }
}
