...
class CreateCostingLifoStoredProcedures extends Migration
{
    /**
     * Run the migrations.
     *
        ...
        FROM
          ...
          FROM
            ...
            INNER JOIN `pr_stock_adjust_details` AS st 
              ...
              AND sad.`delete` = 0 
              AND st.`item_id` = PRODUCT_ID) AS a) AS b ORDER BY b.trans_date DESC";
        \DB::unprepared("DROP PROCEDURE IF EXISTS COSTING_LIFO; CREATE PROCEDURE COSTING_LIFO(IN PROJECT_ID int,IN PRODUCT_ID int) BEGIN ({$sql}); END");
    }
    ...
    {
        \DB::unprepared("DROP PROCEDURE IF EXISTS COSTING_LIFO;");
    }
}
