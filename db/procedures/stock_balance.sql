DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `stock_balance`(IN `pro_` INT, IN `warehouse_` VARCHAR(100), IN `start_` VARCHAR(100), IN `end_` VARCHAR(100), IN `group_` INT, IN `is_zero` INT)
BEGIN
  IF group_ = 0 
  THEN IF is_zero = 0 
  THEN 
